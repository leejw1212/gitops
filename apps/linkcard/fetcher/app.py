...
def fetch():
    ...
    try:
        ...
        return jsonify(ok=False, error=f"HTTP {e.code}", elapsed=round(time.time() - t0, 2)), 502
    except (URLError, TimeoutError) as e:
        return jsonify(ok=False, error=f"가져오지 못했습니다: {e}", elapsed=round(time.time() - t0, 2)), 504
    ...
    app.logger.info("fetched url=%s elapsed=%.2f steps=%s", url, elapsed, steps)
